...
namespace GitFolderKeeper
{
    internal class HistoryRewriter
    {
        ...
        private readonly HashSet<Branch> branchesToRewrite;
        ...
        public HistoryRewriter(
            IRepository repo,
            IEnumerable<Branch> branchesToRewrite,
            RewriteHistoryOptions options)
        {
            this.repo = repo;
            this.options = options;
            this.branchesToRewrite = new HashSet<Branch>(branchesToRewrite);
            targetedCommits = new HashSet<Commit>(branchesToRewrite.SelectMany(c => c.Commits));
        ...
        public void Execute()
        {
            ...
            try
            {
                ...
                var refsToRewrite = this.branchesToRewrite.Select(x => x.Reference).ToList();

                //List<Reference> refsToRewrite = this.branchesToRewrite.Select(x => x.Reference).ToList();
            ...
            where TTarget : class
        {
            ...
            {
                newRefName = "refs/tags/" +
                             options.TagNameRewriter(oldRef.CanonicalName.Substring("refs/tags/".Length),
                                                     false,
            ...
            string backupName = backupRefsNamespace + oldRef.CanonicalName.Substring("refs/".Length) + "_" + DateTime.Now.ToString("yyyyMMddhhmmss");
            if (repo.Refs[backupName] != null)
            {
                //repo.Refs.Remove(backupName);
